...
Die Zeit, die zwischen zwei Rotationen vergeht, wird as \textit{Zykluszeit} bezeichnet.
...
Ziel ist es, einen Produktionsplan $\pi\in J^n$ zu erstellen, der die gesamte Produktionsdauer minimiert.
...
\end{itemize}%
Dabei wird jeweils für $f=g$ $s_{fg} = 0$ angenommen, dass also keine Rüstkosten innerhalb einer Familie auftreten.

Insgesamt gilt es also, neben dem Schedule $\pi$ auch ein Mapping $f:J\rightarrow R$ zu finden, 
das jedem Job $j\in J$ eine Ressource $r\in\rho_j$ zuweist und folgenden Ansprüche genügt:
\begin{itemize}
...
\begin{enumerate}
    \item Zunächst wird der Schedule $\pi$ aufgestellt, ohne Ressourcen und Rüstkosten zu betrachten, 
...
Beispielsweise ist bei Ansatz (1) das Aufstellen von $\pi$ (soweit bekannt) nur in dem Spezialfall $m=2$ 
