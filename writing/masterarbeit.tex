...
Dabei soll zunächst nur der Spezialfall $s_{fg}=s$ betrachtet werden.
...
mit den Größen $n_g, \, g=1,\ldots,\tilde{n}$ in $m$ Bins der Größe $\frac{n}{m}$ untergebracht werden können.
...
Eine Jobgruppe, die nicht geteilt wird, benötigt auch nie mehr als eine Ressource.
Es muss also nur der Fall betrachtet werden, dass es für eine Jobgruppe, die geteilt wird, nur eine Ressource gibt.
Sei $g$ diese Gruppe.

Falls $n_g > \frac{n}{m}$, muss $g$ in jedem Fall geteilt werden.
Bei jeder möglichen Konstellation der Anzahl und Größen der Teilgruppen von $g$ gibt es immer mindestens einen Zeitpunkt,
an dem zwei Jobs von $g$ gleichzeitig in der Anlage sind, was nicht möglich ist, da es nur eine Ressource gibt.
Dieser Fall ist also definitiv nicht zulässig, unabhängig von der Art der Zuteilung der Ressourcen.

Falls andererseits $n_g \leq \frac{n}{m}$, ist es immer möglich, die beiden Teilgruppen, die aus $g$ hervorgehen,
so in den Bins anzuordnen, dass niemals zwei Jobs aus $g$ zur gleichen Zeit in der Anlage sind (vgl. Abbildung \ref{abb:blabla}).
Dies muss allerdings als zusätzliche Bedingung an die nachfolgenden Verfahren, die $C_{\max}$ minimieren (s. Abschnitt \ref{sec:AnordnungDerJobgruppen}),
übergeben werden, da es dort sonst vorkommen kann, dass die beiden Teilgruppen von $g$ dort "`nebeneinander"' angeordnet werden,
so dass mindestens zwei Jobs aus $g$ zur gleichen Zeit in der Anlage wären.
