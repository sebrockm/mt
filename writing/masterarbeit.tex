...
Es müssen daher nur geteite Jobgruppen betrachtet werden, von denen es maximal $m-1$ geben kann.
...
Im vorigen Unterabschnitt wurde festgestellt, dass nur eine geteilte Jobgruppe, für die es nur eine Ressource gibt,
zu Unzulässigkeit führen kann.
...
stellt sich daher die Frage, ob es möglich ist, seine Jobgruppen vorher so Anzuordnen, dass sich nur solche Jobgruppen an den "`Bruchstellen"' befinden, 
...
\begin{proof}
    ...
    Es muss also nur noch ein $\mathcal{NP}$-schweres Problem darauf reduziert werden.
    ...
    "`$\Leftarrow$"': Habe nun umgekehrt $J'$ eine Lösung. Da jeder $1$-Bin eine Jobgruppeder Größe $1$ enthält, die aus $n+1$ hervorgegangen ist,
...
Die Idee des MIPs besteht darin, jeder Jobgruppe $i$ in ihrem Bin $b_i$ einen \textit{Startposition} $s_i$ zuzuweisen.
...
Da der Abstand der dominierenden Maschinen $1$ beträgt, wird der zweite Bin der \textit{Nachbarbin} von Bin 1 genannt.
Bin 3 ist der Nachbarbin von Bin 2, Bin 4 ist der Nachbarbin von Bin 3, \ldots und Bin 1 ist der Nachbarbin von Bin 8.
Wenn der Abstand der dominierenden Maschinen z.B. $3$ betragen würde, dann wäre Bin 4 der Nachbarbin von Bin 1 usw.
...
Die Startposition von $1$ sei nun $s_1=0$ (als allererstes wird also ein Job aus Gruppe $1$ in die Anlage eingelegt)
und die Startposition von $2$ sei $s_2=6$.
Die Überlappung von $1$ und $2$ beträgt also $4$, wie auch in Abbildung \ref{abb:binfixbsp} zu sehen ist.
...
Aus dem Maximum der Prozesszeiten der Jobgruppen und ihrer Überlappung kann dann $C_{\max}$ bestimmt werden.
...
\begin{enumerate}
    ...
    \item $o_{ij} = 0$.
\end{enumerate}
Welche davon die passende ist, muss anhand einiger Hilfsvariblen und Nebenbedingung ermittelt werden.
...
Um festzustellen, welche der Berechnungen für $o_{ij}$ benutzt werden muss, werden binäre Hilfsvariblen $y_{ij}$ und $z_{ij}$ verwendet.
...
Da die Zielfunktion (\ref{objfunc1}) die Überlappung mit dem Maximum der jeweiligen Prozesszeiten multipliziert und die Summe darüber minimiert,
